...
def load_flashar_metadata(path: str) -> Dict[str, Any]:
...
def resolve_flashar_ckpt_path(
    *,
    flashar_ckpt_path: str,
    model_path: str,
    ...
    del vertical_layers
    if not flashar_ckpt_path:
        raise ValueError("flashar_ckpt_path is required for flashar inference loading.")
    flashar_ckpt_path = strip_shard_suffix(flashar_ckpt_path)

    if osp.exists(flashar_ckpt_path):
        return flashar_ckpt_path
    if flashar_ckpt_path.endswith(".pt") and osp.isdir(flashar_ckpt_path[:-3]):
        return flashar_ckpt_path[:-3]

    if flashar_ckpt_path.endswith(".pt"):
        full_candidate = flashar_ckpt_path[:-3] + ".full.pt"
    else:
        full_candidate = flashar_ckpt_path + ".full.pt"
    if osp.exists(full_candidate):
        return full_candidate

    shard_paths = sorted(glob.glob(flashar_ckpt_path + ".rank*.pt"))
    if not shard_paths and flashar_ckpt_path.endswith(".pt"):
        base = flashar_ckpt_path[:-3]
        shard_paths = sorted(glob.glob(base + ".rank*.pt"))
        if shard_paths:
            flashar_ckpt_path = base

    if not shard_paths:
        raise FileNotFoundError(f"flashar ckpt not found: {flashar_ckpt_path}")
    raise FileNotFoundError(
        "Sharded flashar checkpoints are no longer supported in the streamlined mainline. "
        f"Found {len(shard_paths)} shard files for base path: {flashar_ckpt_path}. "
