# -*- coding: utf-8 -*-
# flashar-aware interleaved generation for Emu3.5.
...
from flashar.inference.token_utils import (
...
def _get_flashar_wrapper(cfg, model) -> Emuflashar:
    if hasattr(model, "flashar_wrapper"):
        return model.flashar_wrapper
    flashar_ckpt_path = _resolve_flashar_ckpt_path(
        flashar_ckpt_path=getattr(cfg, "flashar_ckpt_path", ""),
        model_path=cfg.model_path,
        merge_dtype=getattr(cfg, "flashar_merge_dtype", "bf16"),
        fsdp_wrap_policy=getattr(cfg, "flashar_fsdp_wrap_policy", "transformer"),
        fsdp_min_params=int(getattr(cfg, "flashar_fsdp_min_params", 1_000_000)),
        use_vertical_block=getattr(cfg, "flashar_use_vertical_block", None),
        vertical_layers=int(getattr(cfg, "flashar_vertical_layers", 0)),
    )

    state = _safe_torch_load(flashar_ckpt_path)
    flashar_metadata = _load_flashar_metadata(flashar_ckpt_path)
    ...
    flashar_use_vertical = getattr(cfg, "flashar_use_vertical_block", None)
    if flashar_use_vertical is None:
        use_vertical_block = inferred_use_vertical
    else:
        use_vertical_block = bool(flashar_use_vertical)

    cfg_vertical_layers = int(getattr(cfg, "flashar_vertical_layers", 0))
    ...
    if use_vertical_block and vertical_layers <= 0:
        vertical_layers = 1
    cfg_vertical_start_layer = int(getattr(cfg, "flashar_vertical_start_layer", -1))
    ...
    wrapper = Emuflashar(
        ...
        split_backbone=bool(getattr(cfg, "flashar_split_backbone", False)),
    )
    _load_state_with_fuse_compat(wrapper, state, load_desc="load flashar ckpt")
    wrapper = wrapper.to(next(model.parameters()).device)
    wrapper.eval()
    model.flashar_wrapper = wrapper
    return wrapper
...
def non_streaming_generate(
    ...
    flashar_wrapper = _get_flashar_wrapper(cfg, model)
    ...
    while max_new_tokens <= 0 or len(generated) < max_new_tokens:
        ...
        # Parse height/width for flashar grid.
        try:
            if not size_tokens:
        ...
        # flashar decode for visual tokens.
