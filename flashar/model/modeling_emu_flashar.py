...
Usage:
    model = Emuflashar(backbone.model, vocab_size=..., hidden_size=...)
    # Training:
...
class Emuflashar(nn.Module):
    """
    Wrap an AR backbone for flashar training and diagonal-step decoding.
    ...
    def generate(
        ...
        if text_input_ids is None:
            raise ValueError(
                "Emuflashar.generate now requires text_input_ids and always uses KV-cache generation."
            )
