...
from src.utils.model_utils import build_emu3p5, build_emu3p5_flashar_inference
...
def _run_flashar_t2i(
    *,
    ...
    device = next(flashar_wrapper.parameters()).device
    _log_stage("flashar: building single-image prompt tensors")
    ...
    visual_token_offset = int(flashar_wrapper.visual_token_offset)
    diag_steps = int(cfg.target_height) + int(cfg.target_width) - 1
    _log_stage(
        f"flashar: prompt_len={prompt_ids.shape[1]} diag_steps={diag_steps} size={cfg.target_height}x{cfg.target_width} cfg={cfg.classifier_free_guidance}"
    )

    _log_stage("flashar: starting wrapper.generate()")
    infer_start = time.perf_counter()
    grid = flashar_wrapper.generate(
    ...
    sync_cuda()
    infer_seconds = time.perf_counter() - infer_start
    _log_stage(f"flashar: wrapper.generate() finished in {infer_seconds:.2f}s")

    _log_stage("flashar: decoding final image")
    decode_start = time.perf_counter()
    image = decode_flashar_grid_to_image(
        vq_model=vq_model,
        grid=grid,
    ...
    decode_seconds = time.perf_counter() - decode_start
    _log_stage(f"flashar: image decode finished in {decode_seconds:.2f}s")
...
def _run_single_mode(mode_name: str, cfg_path: str, out_root: str) -> Dict[str, Any]:
    ...
    resolved_flashar_ckpt_path = ""
    load_mode = "base_model"
    if mode_name == "flashar":
        load_mode = "flashar_direct_inference_ckpt"
        flashar_wrapper, tokenizer, vq_model, resolved_flashar_ckpt_path = build_emu3p5_flashar_inference(
            cfg.model_path,
            cfg.tokenizer_path,
            cfg.vq_path,
            flashar_ckpt_path=getattr(cfg, "flashar_ckpt_path", ""),
            vq_type=cfg.vq_type,
            model_device=cfg.hf_device,
            vq_device=cfg.vq_device,
            flashar_use_vertical_block=getattr(cfg, "flashar_use_vertical_block", None),
            flashar_vertical_layers=int(getattr(cfg, "flashar_vertical_layers", 0)),
            flashar_vertical_start_layer=int(getattr(cfg, "flashar_vertical_start_layer", -1)),
            flashar_attn_implementation=str(getattr(cfg, "flashar_attn_implementation", "eager")),
            flashar_merge_dtype=str(getattr(cfg, "flashar_merge_dtype", "bf16")),
            flashar_fsdp_wrap_policy=str(getattr(cfg, "flashar_fsdp_wrap_policy", "transformer")),
            flashar_fsdp_min_params=int(getattr(cfg, "flashar_fsdp_min_params", 1_000_000)),
    ...
    if not isinstance(prompt_text, str):
        raise ValueError("benchmark_ar_vs_flashar.py only supports pure t2i string prompts.")
    if getattr(cfg, "use_image", False):
        raise ValueError("benchmark_ar_vs_flashar.py only supports use_image=False.")
    ...
    elif mode_name == "flashar":
        _log_stage("flashar: entering single-image benchmark path")
        run_metrics = _run_flashar_t2i(
            cfg=cfg,
            flashar_wrapper=flashar_wrapper,
            tokenizer=tokenizer,
            vq_model=vq_model,
            prompt_text=prompt_text,
            out_dir=save_path,
    ...
    metrics = {
        ...
        "flashar_ckpt_path": resolved_flashar_ckpt_path or getattr(cfg, "flashar_ckpt_path", ""),
        "load_mode": load_mode,
        "flashar_attn_implementation": str(getattr(cfg, "flashar_attn_implementation", "")),
    ...
    cleanup_cuda(model, flashar_wrapper, tokenizer, vq_model, cfg)
    return metrics
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--flashar_cfg",
        type=str,
        default="configs/benchmark_t2i_flashar.py",
        help="flashar benchmark config path.",
    )
    parser.add_argument(
        "--out_root",
        type=str,
        default="",
        help="Output directory. Defaults to outputs/bench_ar_vs_flashar/<timestamp>.",
    )
...
def main() -> None:
    ...
    if args.out_root:
        ...
        out_root = osp.abspath(osp.join("outputs", "bench_ar_vs_flashar", stamp))
    os.makedirs(out_root, exist_ok=True)

    summaries = []
    for mode_name, cfg_path in (("ar", args.ar_cfg), ("flashar", args.flashar_cfg)):
        ...
        print(
            ...
            f"attn={metrics['flashar_attn_implementation'] or 'default'} "
    ...
    if len(summaries) == 2:
        ar_metrics, flashar_metrics = summaries
        if flashar_metrics["infer_seconds"] > 0:
            speedup = ar_metrics["infer_seconds"] / flashar_metrics["infer_seconds"]
        else:
            speedup = 0.0
        comparison = {
            "ar_infer_seconds": ar_metrics["infer_seconds"],
            "flashar_infer_seconds": flashar_metrics["infer_seconds"],
            "ar_over_flashar_infer_speedup": speedup,
        }
    else:
        comparison = {}
